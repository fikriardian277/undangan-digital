document.addEventListener("DOMContentLoaded", () => {
  ...
  const App = {
    init() {
      ...
      this.initScrollAnimations(); // Fungsi ini akan kita rombak
      this.initCountdown();
      this.initRsvp();
      feather.replace();
    },
    ...
    // --- INI DIA FUNGSI ANIMASI BARU YANG OTOMATIS ---
    // --- INI FUNGSI BARU UNTUK ANIMASI BERULANG ---
      ...
      const observer = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            ...
            // Jika elemen keluar dari layar, hapus class .show untuk mereset animasi
            else {
        ...
        {
          threshold: 0.2, // Elemen dianggap "terlihat" saat 20% masuk layar
    ...
    initCountdown() {
      ...
      if (isNaN(target)) {
        console.error("FORMAT TANGGAL SALAH di CONFIG.event.isoDate.");
        return;
      }
